...
**User**
: A user is an entity that can authenticate themselves.
...
: A Role is a collection of permissions.
  ...
  *Global Roles*
  : Global roles are assigned to a user or group via Kotti's user management screens.
    ...
    You should use them very rarely — maybe only assign the "Administrator" role to the "Administrator" group.
...
: The workflow keeps track of the current state of each object lifecycle to manage content security.
  ...
  Kotti's workflow implementation is based on [repoze.workflow](http://docs.repoze.org/workflow/).
...
You can see an example here: [workflow.zcml](https://github.com/Kotti/Kotti/blob/master/kotti/workflow.zcml).
...
Last you have to tell Kotti to register your new custom workflow by including our ZCML file:
