...
The [Installation](../../guides/getting-started/installation.md) section explains how to get hold of a sample configuration file.
...
Only the settings in **bold** are required. The rest have defaults.
...
| **sqlalchemy.url** | [SQLAlchemy database URL](http://www.sqlalchemy.org/docs/core/engines.html#database-urls) |
...
| kotti.depot_mountpoint | Configure the mountpoint for the blob storage. See [Blobs](blobs.md) for details. |
| kotti.depot_replace_wsgi_file_wrapper | Replace your WSGI server's file wrapper with `pyramid.response.FileIter`. |
| kotti.depot.\*.\* | Configure the blob storage. See [Blobs](blobs.md) for details. |
...
| kotti.sanitize_on_write | Configure [sanitizers](sanitizers.md) to be used on write access to resource objects |
| kotti.sanitizers | Configure available [sanitizers](sanitizers.md) |
...
## Override templates (`kotti.asset_overrides`) {#asset-overrides}
...
As an example, imagine that we wanted to override Kotti's master layout template.
...
Here's an example. Let's install the [kotti_twitter](http://pypi.python.org/pypi/kotti_twitter) extension and add a Twitter profile widget to the right column of all pages.
...
Read more about [including packages using 'pyramid.includes'](http://readthedocs.org/docs/pyramid/en/1.3-branch/narr/environment.html#including-packages) in the Pyramid documentation.

### kotti.available_types {#kottia-available-types}
...
You can provide your own search function in an add-on and register this in your INI file.
...
An add-on that defines an alternative search function is [kotti_solr](http://pypi.python.org/pypi/kotti_solr), which provides an integration with the [Solr](http://lucene.apache.org/solr/) search engine.
...
The list of available languages is [here](https://github.com/Kotti/Kotti/tree/master/kotti/locale).
...
These settings correspond to [pyramid.authentication.AuthTktAuthenticationPolicy](http://docs.pylonsproject.org/projects/pyramid/dev/api/authentication.html) and [pyramid.authorization.ACLAuthorizationPolicy](http://docs.pylonsproject.org/projects/pyramid/dev/api/authorization.html) being used.
...
Kotti normalizes document titles to URLs by replacing language specific characters like umlauts or accented characters with their ASCII equivalents.
...
You may also replace the default component used for URL normalization by setting the `kotti.url_normalizer` configuration variable.
...
Kotti provides a built-in navigation widget, which is disabled by default.
...
The add-on [kotti_navigation](http://pypi.python.org/pypi/kotti_navigation) provides also a navigation widget with more features.
...
Check the documentation of [kotti_navigation](http://pypi.python.org/pypi/kotti_navigation) for more options.
