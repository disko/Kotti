...
This recipe describes how to configure Kotti to require users to log in
before they can view any of your site's pages.

To achieve this, we'll have to set our site's ACL. A custom populator
will help us do that (see [Configuration](configuration.md)).

Remember that the default site ACL gives `view` privileges to every
user, including anonymous (see [Security](security.md)). We'll thus
