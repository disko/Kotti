...
A number of them are available on [PyPI](http://pypi.python.org/pypi?%3Aaction=search&term=kotti_&submit=search/).
They include [kotti_media](http://pypi.python.org/pypi/kotti_media/), for adding a set of video and audio content types to a site, [kotti_gallery](http://pypi.python.org/pypi/kotti_gallery/), for adding a photo album content type, [kotti_blog](http://pypi.python.org/pypi/kotti_blog/), for blog and blog entry content types, etc.

The add-on we will make, `kotti_mysite`, will be just like those, in that it will be a proper Python package created with the same command line tools used to make `kotti_media`, `kotti_blog`, and the others.
...
So, we are working in the `mysite` directory, a virtualenv, as described in [Installation](installation.md).
...
[cookiecutter](https://github.com/audreyr/cookiecutter), using the `kotti-cookiecutter` scaffolding.
...
See [kotti-cookiecutter](https://github.com/Kotti/kotti-cookiecutter) for further info about scaffolding options.
...
To do this, open the `app.ini` file, which you downloaded during [Installation](installation.md).
...
Kotti uses [fanstatic](http://www.fanstatic.org/) for managing its static resources.
...
[fanstatic](http://www.fanstatic.org/) has a number of cool features -- you may want to check out their homepage to find out more.
...
In the [next part](tut-2.md) of the tutorial, we'll add our first content types, and add forms for them.
